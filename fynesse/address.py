...
def fetch_all_training_data(target_lat, target_long, feature_range_width, feature_range_height, property_range_width, property_range_height, category_list, property_type, date, days_since=365):
    ...
    """
    category_pois_dict = fetch_features_outer_bbox(target_lat, target_long, feature_range_width, feature_range_height, property_range_width, property_range_height, category_list)
    print("fetched all features")
    price_df = fetch_pp_and_pc_joined_area(target_lat, target_long, date, property_range_height, property_range_width, days_since)
    print("fetched pricing data")
    price_df = remove_price_tails(price_df, 0.025)
    ...
    for category in category_list:
        ...
        feature_df = fetch_geo_features_nearby_from_df(float(training_df["lattitude"].iloc[i]), float(training_df["longitude"].iloc[i]), feature_range_width, feature_range_height, category_pois_dict[category])
        training_df[f"n_{category}"].iloc[i] = len(feature_df)
        training_df[f"shortest_distance_{category}"].iloc[i] = float(calculate_closest_feature(float(training_df["lattitude"].iloc[i]), float(training_df["longitude"].iloc[i]), feature_df, feature_range_width*(2**(1/2))))
...
def fetch_prediction_data(target_lat, target_long, feature_range_width, feature_range_height, category_pois_dict, date, days_since=365):
    ...
    for category in category_pois_dict.keys():
        feature_df = fetch_geo_features_nearby_from_df(float(target_lat), float(target_long), feature_range_width, feature_range_height, category_pois_dict[category])
        pred_row[f"n_{category}"] = len(feature_df)
        pred_row[f"shortest_distance_{category}"] = float(calculate_closest_feature(float(target_lat), float(target_long), feature_df, feature_range_width*(2**(1/2))))
